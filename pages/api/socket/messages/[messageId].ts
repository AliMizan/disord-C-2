import { currentProfilePages } from "@/lib/currenr-profile-pages";
...
export default async function handler(req:NextApiRequest,res:NextApiResponseServerIo) {
    ...
    try {
        ...
        if(!channel){
            return res.status(404).json({error:"Channel Is Missing"});
        };

        const member = server.members.find((member) => member.profileId === profile.id);

        if(!member){
            return res.status(404).json({error:"Member Is Missing"});
        };
        ...
        if(!canModify){
            return res.status(401).json({error:"Unaurhorised"});
        };
